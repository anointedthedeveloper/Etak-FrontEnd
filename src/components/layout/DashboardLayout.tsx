    ...
    `}>
      ...
      <div className="flex items-center gap-2 px-5 py-5 border-b border-gray-100">
        <Link to="/" className="flex items-center gap-2">
          <img src="/logo.png" alt="Etak Travels" className="h-12 w-12 object-contain" />
          <div>
        ...
        {mobile && (
          <button onClick={() => setSidebarOpen(false)} className="ml-auto p-1 text-[#667085]">
            <X size={20} />
          </button>
        )}
      </div>
      ...
      {/* Bottom promo card */}
      <div className="m-3 rounded-2xl bg-gradient-to-br from-[#101B46] to-[#45419A] p-4 text-white">
        <Plane size={22} className="text-[#08A9E0] mb-3" />
      ...
      <button
        ...
        className="flex items-center gap-3 px-6 py-4 text-sm text-red-500 hover:bg-red-50 transition-colors border-t border-gray-100"
      >
    ...
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 flex items-center gap-4 shrink-0">
          {/* Mobile menu toggle */}
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-1.5 text-[#667085]">
            <Menu size={22} />
          </button>
          ...
          <div className="flex-1 max-w-md relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl bg-gray-50 border border-gray-100 text-sm text-[#172033] placeholder-[#667085] focus:outline-none focus:ring-2 focus:ring-[#08A9E0] focus:border-transparent"
            />
          </div>
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 pl-1 pr-3 py-1 rounded-xl hover:bg-gray-50 transition-colors"
              >
                <div className="w-8 h-8 rounded-full bg-[#08A9E0] flex items-center justify-center text-white text-xs font-bold shrink-0">
                  {initials}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-sm font-semibold text-[#172033] leading-none">
                    {user?.firstName} {user?.lastName}
                  </p>
                  <p className="text-xs text-[#667085] leading-none mt-0.5">
                    {user?.email?.split('@')[0]}
                  </p>
                </div>
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50">
                  <div className="px-4 py-2.5 border-b border-gray-100">
                    <p className="text-sm font-semibold text-[#172033]">{user?.firstName} {user?.lastName}</p>
